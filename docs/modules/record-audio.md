...
**Re-list flushes the cache before probing**, **multi-pass enumeration** (3 × 100 ms), and **unified across input and output** — see [`playback-audio` §3.3.1](./playback-audio.md#331-device-handle-caching--devicecatalog) for the full rationale. The third one matters specifically for the recorder: before unification, listing outputs (player catalog) populated `cpal::Device`s that held both PCM directions, blocking the recorder catalog's input probe of the same physical device. The shared `octave-audio-devices::DeviceCatalog` removes the conflict — one Device per hardware, two engines borrowing different stream directions from it.
